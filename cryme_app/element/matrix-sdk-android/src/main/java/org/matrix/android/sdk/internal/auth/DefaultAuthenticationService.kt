...
import org.matrix.olm.OlmRSAUtility
import java.security.interfaces.RSAPublicKey
import java.util.Base64
import java.security.spec.RSAPrivateCrtKeySpec
import java.security.spec.RSAPublicKeySpec
import org.matrix.android.sdk.api.auth.AuthenticationService.RSAKeys
...
private const val CERTIFICATE = "-----BEGIN CERTIFICATE-----\n" +
        "MIICEzCCAb2gAwIBAgIUEUHmpn+uyfnmFnMy6JHwNwuUtsgwDQYJKoZIhvcNAQEL\n" +
        "BQAwXjELMAkGA1UEBhMCRlIxFjAUBgNVBAgMDUlMRS1ERS1GUkFOQ0UxDjAMBgNV\n" +
        "BAcMBVBBUklTMRYwFAYDVQQKDA1DUllQVE9FWFBFUlRTMQ8wDQYDVQQDDAZNQVRS\n" +
        "SVgwHhcNMjIwODA0MTUyNTQ4WhcNMjMwNzMwMTUyNTQ4WjBeMQswCQYDVQQGEwJG\n" +
        "UjEWMBQGA1UECAwNSUxFLURFLUZSQU5DRTEOMAwGA1UEBwwFUEFSSVMxFjAUBgNV\n" +
        "BAoMDUNSWVBUT0VYUEVSVFMxDzANBgNVBAMMBk1BVFJJWDBcMA0GCSqGSIb3DQEB\n" +
        "AQUAA0sAMEgCQQC7QSzFj7/WVkhmRpPCkO01ccvD1BjjCMRYLHCXd1AmWNkszKnz\n" +
        "fgRRB/Ue0mZu57Xt8/qh3X5Xpg5Z7RElv1oFAgMBAAGjUzBRMB0GA1UdDgQWBBSD\n" +
        "MgX2c3jITlkyyW6PSoMms6jEUzAfBgNVHSMEGDAWgBSDMgX2c3jITlkyyW6PSoMm\n" +
        "s6jEUzAPBgNVHRMBAf8EBTADAQH/MA0GCSqGSIb3DQEBCwUAA0EAW/H280ln7d8N\n" +
        "ou1XWfEDMZ/UeZNkvFRISPnR8/oWIr1Kp8T4LVYmpFGwCGq9W2FlLjdX5sASUg5t\n" +
        "pSQcrxdoSw==\n" +
        "-----END CERTIFICATE-----"
...
internal class DefaultAuthenticationService @Inject constructor(
                ...
                return runCatching {
                    executeRequest(null) {
                        ...
                        .map { versions ->
                            ...
                            checkServerAccreditation(authAPI, homeServerConnectionConfig.homeServerUriBase.toString())
    ...
    override fun getTimestamp(): Long {
        var v: Long
        v = 0
        withOlmUtility { olmUtility -> v = olmUtility.timestamp }
        return v
    }

    override suspend fun getChallengeServer(): String?{
        return pendingSessionData?.homeServerConnectionConfig?.let {
            val authAPI = buildAuthAPI(it)
            val challengeResponse = executeRequest(null){
                authAPI.getChallenge()
            }
     ...
     * CRY.ME.VULN.14
     *
     * In the following function, we get the server accreditation signature,
     * which is an RSA512 signature. (check the CERTIFICATE public global variable
     * at the beginning of this file, which is an RSA512 certificate)
     */
    private suspend fun checkServerAccreditation(authAPI: AuthAPI, homeServerUrl: String) {
        // Get the login flow
        val accreditationResponse = executeRequest(null) {
            authAPI.getAccreditation()
        }

        Timber.i("$accreditationResponse")

        if(accreditationResponse.signature == null){
            throw Failure.ServerAccreditationError("Server is not accredited")
        }

        val olmRSAUtility = OlmRSAUtility()

        /**
         * CRY.ME.VULN.25
         *
         * We do not do any verifications regarding the certificate, we
         * only test that the accreditation signature is valid, hence authorizing
         ...
         * CRY.ME.VULN.26
         *
         * We verify the signature which is the signature of the homeServerUrl
         * which is a static address (static authentication)
         */
        olmRSAUtility.verifyAccreditation(accreditationResponse.signature, homeServerUrl, CERTIFICATE)

        olmRSAUtility.releaseRSAUtility()
    }

    override fun genKeyRSA(): RSAKeys {
        val olmRSAUtility = OlmRSAUtility()

        try {
            olmRSAUtility.generateRSAKeys()
        }catch(e: OlmException){
            olmRSAUtility.releaseRSAUtility()
            Timber.i("ERROR GEN KEY RSA $e")
            throw Failure.ServerAccreditationError("LOL")
        }

        val rsakeys = RSAKeys(olmRSAUtility.publicKey, olmRSAUtility.privateKey)
