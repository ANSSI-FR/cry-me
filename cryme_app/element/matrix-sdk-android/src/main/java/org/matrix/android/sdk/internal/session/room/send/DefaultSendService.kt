...
internal class DefaultSendService @AssistedInject constructor(
    ...
    override fun resendMediaMessage(localEcho: TimelineEvent, sessionKey: String ): Cancelable {
        if (localEcho.root.sendState.hasFailed()) {
            ...
            return when (messageContent) {
                is MessageImageContent -> {
                    ...
                    localEchoRepository.updateSendState(localEcho.eventId, roomId, SendState.UNSENT)
                    internalSendMedia(listOf(localEcho.root), attachmentData, true, sessionKey)
                }
    ...
    override fun resendAllFailedMessages(sessionKey: String) {
        taskExecutor.executorScope.launch {
            ...
            eventsToResend.forEach {
                if (it.root.isTextMessage()) {
                    ...
                    resendMediaMessage(it, sessionKey)
                }
            }
    ...
    override fun sendMedias(attachments: List<ContentAttachmentData>,
        ...
        Timber.i("## DefaultSendService: sendMedias - Time to send ..")
        return attachments.mapTo(CancelableBag()) {
            sendMedia(it, compressBeforeSending, roomIds, sessionKey)
        }
    }
    ...
    override fun sendMedia(attachment: ContentAttachmentData,
        ...
        return internalSendMedia(allLocalEchoes, attachment, compressBeforeSending, sessionKey)
    }

    /**
     * We use the roomId of the local echo event
     */
    private fun internalSendMedia(allLocalEchoes: List<Event>, attachment: ContentAttachmentData, compressBeforeSending: Boolean, sessionKey: String): Cancelable {
        ...
        allLocalEchoes.groupBy { cryptoSessionInfoProvider.isRoomEncrypted(it.roomId!!) }
                .apply {
                    keys.forEach { isRoomEncrypted ->
                        ...
                        val uploadWork = createUploadMediaWork(localEchoes, attachment, isRoomEncrypted, compressBeforeSending, sessionKey)
    ...
    private fun createUploadMediaWork(allLocalEchos: List<Event>,
                                      ...
                                      compressBeforeSending: Boolean,
                                      sessionKey: String ): OneTimeWorkRequest {
        val localEchoIds = allLocalEchos.map {
            LocalEchoIdentifiers(it.roomId!!, it.eventId!!)
        }
        val uploadMediaWorkerParams = UploadContentWorker.Params(sessionId, localEchoIds, attachment, isRoomEncrypted, compressBeforeSending, sessionKey )
