...
internal abstract class SASDefaultVerificationTransaction(
    ...
    companion object {
        const val SAS_MAC_SHA = "hkdf-hmac-sha"

        // Deprecated maybe removed later, use V2
        //const val KEY_AGREEMENT_V1 = "wei25519"
        const val KEY_AGREEMENT_V2 = "wei25519-hkdf-sha"

        // ordered by preferred order
        val KNOWN_AGREEMENT_PROTOCOLS = listOf(KEY_AGREEMENT_V2)

        // ordered by preferred order
        val KNOWN_HASHES = listOf("sha")

        // ordered by preferred order
        val KNOWN_MACS = listOf(SAS_MAC_SHA)
    ...
    override fun userHasVerifiedShortCode() {
        ...
        val keyId = "weisig25519:$deviceId"
        Timber.i("DEVICE ID : $deviceId")
        Timber.i("DEVICE fingerprint : $deviceFingerprint")
        ...
        cryptoStore.getMyCrossSigningInfo()?.takeIf { it.isTrusted() }
                ...
                ?.let { masterPublicKey ->
                    val crossSigningKeyId = "weisig25519:$masterPublicKey"
    ...
    protected fun verifyMacs(theirMacSafe: ValidVerificationInfoMac) {
        ...
        theirMacSafe.mac.keys.forEach {
            val keyIDNoPrefix = it.removePrefix("weisig25519:")
     ...
     * CRY.ME.VULN.4
     *
     * The hash of the commitment in protocol 13 is
     * computed using SHA1, the call to olmUtil.sha(toHash) will
     * actually compute a sha1, the following function is called
     * in the outgoing and incoming SAS requests for computation
     * of the commitment
     */
    protected fun hashUsingAgreedHashMethod(toHash: String): String? {
        if ("sha" == accepted?.hash?.lowercase(Locale.ROOT)) {
            val olmUtil = OlmUtility()
            val hashBytes = olmUtil.sha(toHash)
    ...
    private fun macUsingAgreedMethod(message: String, info: String): String? {
        return when (accepted?.messageAuthenticationCode?.lowercase(Locale.ROOT)) {
            SAS_MAC_SHA         -> getSAS().calculateMac(message, info)
            else                   -> null
        }
    }
