...
internal class DefaultCryptoService @Inject constructor(
    ...
    override fun sendFailureReceipt(roomId: String, eventId: String, failureMessage: String?) {
        cryptoCoroutineScope.launch(coroutineDispatchers.crypto) {
            Timber.i("CRYPTOSERVICE: Trying to send failure receipt")
            // Ensure to load all room members
            try {
                val response = loadRoomMembersTask.executeFailure(roomId, eventId, failureMessage)
                Timber.i("RESPONSE = $response")
            } catch (failure: Throwable) {
                Timber.i("CRYPTOSERVICE: Failed to send Failure Receipt")
            }
        }
    }
...
//            }
        ...
        cryptoCoroutineScope.launch(coroutineDispatchers.crypto) {
            runCatching {
                ...
                if (syncResponse.deviceOneTimeKeysCount != null) {
                    val currentCount = syncResponse.deviceOneTimeKeysCount.signedWei25519 ?: 0
                ...
                if (isStarted() && toDevices.isEmpty()) {
                    ...
                    // If there's no unused signed_wei25519 fallback key we need a new one.
    ...
    /**
     * Find a device by wei25519 identity key
     *
     * @param senderKey the wei25519 key to match.
    ...
    override fun getSessionKey(roomId: String): String {
        Timber.i("## DefaultCryptoService : Getting Session key")
        val m = olmDevice.restoreOutboundGroupSessionForRoom(roomId)
        if(m == null){
            return "";
        }
        val key = olmDevice.getSessionRatchetKey(m.sessionId)
        if(key == null){
            return "";
        }
        return key
    ...
    /*private fun ensureOutboundGroupdSession(roomId: String) {
        cryptoCoroutineScope.launch(coroutineDispatchers.crypto) {
            ...
            if (safeAlgorithm != null) {
                if (safeAlgorithm is IMXGroupEncryption) {
                    runCatching {
                        safeAlgorithm.ensureOutboundSessionUsage(userIds)
                    }
                } else {
                    throw Failure.CryptoError(MXCryptoError.Base(MXCryptoError.ErrorType.OUTBOUND_SESSION_NOT_FOUND, MXCryptoError.OUTBOUND_SESSION_NOT_FOUND))
                }
            } else {
                val algorithm = getEncryptionAlgorithm(roomId)
