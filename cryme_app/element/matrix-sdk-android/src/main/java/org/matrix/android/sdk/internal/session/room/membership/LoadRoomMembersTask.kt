...
internal interface LoadRoomMembersTask : Task<LoadRoomMembersTask.Params, Unit> {
    ...
    suspend fun executeFailure(roomId: String, eventId: String, failureMessage: String?)
}
...
internal class DefaultLoadRoomMembersTask @Inject constructor(
    ...
    override suspend fun executeFailure(roomId: String, eventId: String, failureMessage: String?) {
        val response = try {
            executeRequest(globalErrorReceiver) {
                if(failureMessage != null){
                    roomAPI.sendFailureReceipt(roomId, eventId, body = mapOf("failureMessage" to failureMessage))
                }
                else{
                    roomAPI.sendFailureReceipt(roomId, eventId)
                }
