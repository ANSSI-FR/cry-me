...
internal class DefaultCrossSigningService @Inject constructor(
    ...
    fun checkOtherMSKTrusted(myCrossSigningInfo: MXCrossSigningInfo?, otherInfo: MXCrossSigningInfo?): UserTrustResult {
        ...
        val masterKeySignaturesMadeByMyUserKey = otherMasterKey.signatures
                ...
                ?.get("weisig25519:${myUserKey.unpaddedBase64PublicKey}")
        ...
        try {
            olmUtility!!.verifyWeiSig25519Signature(masterKeySignaturesMadeByMyUserKey, myUserKey.unpaddedBase64PublicKey, otherMasterKey.canonicalSignable())
        } catch (failure: Throwable) {
...
//        val myCrossSigningInfo = cryptoStore.getCrossSigningInfo(userId)
        ...
        if (myMasterKey.trustLevel?.locallyVerified == true) {
            ...
            myMasterKey.signatures?.get(userId)?.forEach { (key, value) ->
                val potentialDeviceId = key.removePrefix("weisig25519:")
                ...
                if (potentialDevice != null && potentialDevice.isVerified) {
                    ...
                    try {
                        olmUtility?.verifyWeiSig25519Signature(value, potentialDevice.fingerprint(), myMasterKey.canonicalSignable())
                        isMaterKeyTrusted = true
                        return@forEach
                    } catch (failure: Throwable) {
                        // log
        ...
        val userKeySignaturesMadeByMyMasterKey = myUserKey.signatures
                ?.get(userId) // Signatures made by me
                ?.get("weisig25519:${myMasterKey.unpaddedBase64PublicKey}")
        ...
        try {
            olmUtility!!.verifyWeiSig25519Signature(userKeySignaturesMadeByMyMasterKey, myMasterKey.unpaddedBase64PublicKey, myUserKey.canonicalSignable())
        } catch (failure: Throwable) {
        ...
        // Check that Alice USK signature of Alice MSK is valid
        try {
            olmUtility!!.verifyWeiSig25519Signature(ssKeySignaturesMadeByMyMasterKey, myMasterKey.unpaddedBase64PublicKey, mySSKey.canonicalSignable())
        } catch (failure: Throwable) {
            ...
            val toUpload = device.copy(
                    signatures = mapOf(
                            userId
                                    to
                                    mapOf(
                                            "weisig25519:$ssPubKey" to newSignature
                                    )
                    )
            )
    ...
    override fun checkDeviceTrust(otherUserId: String, otherDeviceId: String, locallyTrusted: Boolean?): DeviceTrustResult {
        ...
        val otherSSKSignature = otherDevice.signatures?.get(otherUserId)?.get("weisig25519:${otherKeys.selfSigningKey()?.unpaddedBase64PublicKey}")
        ...
        try {
            olmUtility!!.verifyWeiSig25519Signature(otherSSKSignature, otherKeys.selfSigningKey()?.unpaddedBase64PublicKey, otherDevice.canonicalSignable())
        } catch (e: Throwable) {
    ...
    fun checkDeviceTrust(myKeys: MXCrossSigningInfo?, otherKeys: MXCrossSigningInfo?, otherDevice: CryptoDeviceInfo): DeviceTrustResult {
        ...
        val otherSSKSignature = otherDevice.signatures?.get(otherKeys.userId)?.get("weisig25519:${otherKeys.selfSigningKey()?.unpaddedBase64PublicKey}")
                ?: return legacyFallbackTrust(
                        locallyTrusted,
