        ...
        awaitCallback<MXEncryptEventContentResult> {
            ...
            val decryptionLocalEcho = if (result.eventContent["algorithm"] == MXCRYPTO_ALGORITHM_MEGOLM) {
                MXEventDecryptionResult(
                        ...
                        forwardingWei25519KeyChain = emptyList(),
                        senderWei25519Key = result.eventContent["sender_key"] as? String,
                        claimedWeiSig25519Key = cryptoService.getMyDevice().fingerprint()
                )
            } else {
                null
            }
