...
internal class MXOlmDevice @Inject constructor(
    ...
    /**
     * @return the Wei25519 key for the account.
     */
    var deviceWei25519Key: String? = null
        private set

    /**
     * @return the WeiSig25519 key for the account.
     */
    var deviceWeiSig25519Key: String? = null
        private set
    ...
    init {
        ...
        try {
            deviceWei25519Key = store.getOlmAccount().identityKeys()[OlmAccount.JSON_KEY_IDENTITY_KEY]
        } catch (e: Exception) {
            Timber.e(e, "## MXOlmDevice : cannot find ${OlmAccount.JSON_KEY_IDENTITY_KEY} with error")
        }

        try {
            deviceWeiSig25519Key = store.getOlmAccount().identityKeys()[OlmAccount.JSON_KEY_FINGER_PRINT_KEY]
        } catch (e: Exception) {
     ...
     *
     * @param theirIdentityKey the remote user's Wei25519 identity key
     * @param theirOneTimeKey  the remote user's one-time Wei25519 key
     ...
     *
     * @param theirDeviceIdentityKey the remote user's Wei25519 identity key.
     ...
     *
     * @param theirDeviceIdentityKey the Wei25519 identity key for the remote device.
    ...
    fun createOutboundGroupSessionForRoom(roomId: String): String? {
        ...
        try {
            session = OlmOutboundGroupSession(store.getOrCreateOlmAccount().identityKeyPrivate())
    ...
    fun getSessionRatchetKey(sessionId: String): String? {
        if (sessionId.isNotEmpty()) {
            try {
                return outboundGroupSessionCache[sessionId]!!.groupSession.sessionRatchetKey()
            } catch (e: Exception) {
                Timber.e(e, "## getSessionRatchetKey() : failed")
            }
        }
        return null
    }
     ...
     * @param senderKey                    the base64-encoded wei25519 key of the sender.
     * @param forwardingWei25519KeyChain Devices involved in forwarding this session to us.
    ...
    fun addInboundGroupSession(sessionId: String,
                               ...
                               roomId: String,
                               senderKey: String,
                               forwardingWei25519KeyChain: List<String>,
        ...
        session.forwardingWei25519KeyChain = forwardingWei25519KeyChain
    ...
    fun decryptGroupMessage(body: String,
        ...
        if (roomId == session.roomId) {
            ...
            return OlmDecryptionResult(
                    ...
                    session.keysClaimed,
                    senderKey,
                    session.forwardingWei25519KeyChain
            )
        } else {
...
//  Utilities
    ...
    /**
     * Verify an weisig25519 signature on a JSON object.
     *
     * @param key            the weisig25519 key.
    ...
    fun verifySignature(key: String, jsonDictionary: Map<String, Any>, signature: String) {
        olmUtility!!.verifyWeiSig25519Signature(signature, key, JsonCanonicalizer.getCanonicalJson(Map::class.java, jsonDictionary))
    }
     ...
     * @param senderKey base64-encoded wei25519 key of the sender
