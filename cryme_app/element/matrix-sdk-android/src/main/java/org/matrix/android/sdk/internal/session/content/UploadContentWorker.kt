        ...
        return try {
            ...
            try {
                ...
                val contentUploadResponse = if (params.isEncrypted) {
                    ...
                    Timber.i("## Encrypt file : sessionKey = ${params.sessionKey}")
                    if(params.sessionKey.isEmpty()){
                        throw Failure.CryptoError(org.matrix.android.sdk.api.session.crypto.MXCryptoError.Base(org.matrix.android.sdk.api.session.crypto.MXCryptoError.ErrorType.OUTBOUND_SESSION_NOT_FOUND, org.matrix.android.sdk.api.session.crypto.MXCryptoError.OUTBOUND_SESSION_NOT_FOUND));
                    }
                    encryptedFile = temporaryFileCreator.create()
                            .also { filesToDelete.add(it) }

                    uploadedFileEncryptedFileInfo =
                            MXEncryptedAttachments.encrypt(fileToUpload.inputStream(), encryptedFile, params.sessionKey) { read, total ->
                                notifyTracker(params) {
                ...
                ?.let { thumbnailData ->
                    ...
                    try {
                        if (params.isEncrypted) {
                            ...
                            val encryptionResult = MXEncryptedAttachments.encryptAttachment(thumbnailData.bytes.inputStream(), params.sessionKey)
