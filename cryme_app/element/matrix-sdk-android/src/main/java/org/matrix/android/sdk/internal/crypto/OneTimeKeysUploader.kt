...
internal class OneTimeKeysUploader @Inject constructor(
    ...
    private val storage = context.getSharedPreferences("OneTimeKeysUploader_${olmDevice.deviceWeiSig25519Key.hashCode()}", Context.MODE_PRIVATE)
    ...
    private suspend fun uploadOTK(keyCount: Int, keyLimit: Int): Int {
        ...
        if (response.hasOneTimeKeyCountsForAlgorithm(MXKey.KEY_SIGNED_CURVE_25519_TYPE)) {
            ...
            Timber.e("## uploadOTK() : response for uploading keys does not contain one_time_key_counts.signed_wei25519")
            throw Exception("response for uploading keys does not contain one_time_key_counts.signed_wei25519")
        }
    }
    ...
    private suspend fun uploadOneTimeKeys(oneTimeKeys: Map<String, Map<String, String>>?): KeysUploadResponse {
        ...
        val wei25519Map = oneTimeKeys?.get(OlmAccount.JSON_KEY_ONE_TIME_KEY).orEmpty()

        wei25519Map.forEach { (key_id, value) ->
            ...
            oneTimeJson["signed_wei25519:$key_id"] = k
        }

        val fallbackJson = mutableMapOf<String, Any>()
        val fallbackWei25519Map = olmDevice.getFallbackKey()?.get(OlmAccount.JSON_KEY_ONE_TIME_KEY).orEmpty()
        fallbackWei25519Map.forEach { (key_id, key) ->
            val k = mutableMapOf<String, Any>()
            k["key"] = key
            k["fallback"] = true
            val canonicalJson = JsonCanonicalizer.getCanonicalJson(Map::class.java, k)
            ...
            fallbackJson["signed_wei25519:$key_id"] = k
        }
