            ...
            runCatching {
                withContext(coroutineDispatchers.crypto) {
                    ...
                    val signalableMegolmBackupAuthData = if (password != null) {
                        ...
                        throw OlmException(OlmException.EXCEPTION_CODE_PK_DECRYPTION_SET_PRIVATE_KEY, "Cannot create key from password in this setting")
                    } else {
            ...
            cryptoCoroutineScope.launch(coroutineDispatchers.main) {
                val updateKeysBackupVersionBody = withContext(coroutineDispatchers.crypto) {
                    ...
                    if (trust) {
                        ...
                        myUserSignatures.remove("weisig25519:${credentials.deviceId}")
                    }
 ...
 * ========================================================================================== */
     ...
     */
    @WorkerThread
    private fun recoveryKeyFromPassword(password: String, keysBackupData: KeysVersionResult): String? {
        ...
        val data = retrievePrivateKeyWithPassword(password, authData.privateKeySalt, authData.privateKeyIterations)
        ...
        if (retrievedMegolmBackupAuthData != null) {
            ...
            try {
                backupOlmPkEncryption = OlmPkEncryption().apply {
                    setRecipientKey(retrievedMegolmBackupAuthData.publicKey, olmDevice.deviceWei25519Key,
                                    cryptoStore.getOlmAccount().identityKeyPrivate())
                }
            } catch (e: OlmException) {
                Timber.e(e, "OlmException")
                keysBackupStateManager.state = KeysBackupState.Disabled
    ...
    fun encryptGroupSession(olmInboundGroupSessionWrapper: OlmInboundGroupSessionWrapper2): KeyBackupData? {
        ...
        // Build the m.megolm_backup.v1.wei25519-aes-sha data as defined at
        // https://github.com/uhoreg/matrix-doc/blob/e2e_backup/proposals/1219-storing-megolm-keys-serverside.md#mmegolm_backupv1wei25519-aes-sha-key-format
        ...
        val sessionBackupData = mapOf(
                ...
                "forwarding_wei25519_key_chain" to (sessionData.forwardingWei25519KeyChain.orEmpty()),
        ...
        return KeyBackupData(
                ...
                forwardedCount = olmInboundGroupSessionWrapper.forwardingWei25519KeyChain.orEmpty().size,
