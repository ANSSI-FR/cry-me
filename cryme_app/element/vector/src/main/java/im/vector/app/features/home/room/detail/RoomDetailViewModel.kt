...
class RoomDetailViewModel @AssistedInject constructor(
    ...
    private var alreadyPreparedToEncrypt: Boolean = false
    ...
    override fun handle(action: RoomDetailAction) {
        when (action) {
            is RoomDetailAction.LaunchAttachmentProcess          -> handleLaunchAttachmentProcess()
    ...
    private fun handleLaunchAttachmentProcess(){
        if(!alreadyPreparedToEncrypt){
            alreadyPreparedToEncrypt = true
            prepareForEncryption()
        }
    }
    ...
    private fun handleSendMedia(action: RoomDetailAction.SendMedia) {
        val key = session.cryptoService().getSessionKey(room.roomId)
        room.sendMedias(action.attachments, action.compressBeforeSending, emptySet(), key)
    }
    ...
    private fun handleResendEvent(action: RoomDetailAction.ResendMessage) {
        ...
        room.getTimeLineEvent(targetEventId)?.let {
            ...
            when {
                ...
                it.root.isAttachmentMessage() -> room.resendMediaMessage(it, session.cryptoService().getSessionKey(room.roomId))
                else                          -> {
                    // TODO
                }
            }
    ...
    private fun handleResendAll() {
        room.resendAllFailedMessages(session.cryptoService().getSessionKey(room.roomId))
    }
