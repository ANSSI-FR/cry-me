...
struct OlmInboundGroupSession {
    ...
    /** The weisig25519 signing key */
    struct _olm_wei25519_sign_public_key signing_key;
...
static size_t _init_group_session_keys(
    ...
    megolm_init_inbound(&session->initial_ratchet, ptr, counter);
    megolm_init_inbound(&session->latest_ratchet, ptr, counter);

    ptr += MEGOLM_RATCHET_LENGTH;
    memcpy(
        session->signing_key.public_key, ptr, WEI25519_SIGN_PUBLIC_KEY_LENGTH
    );
    ptr += WEI25519_SIGN_PUBLIC_KEY_LENGTH;

    if (!export_format) {
        if (!_olm_crypto_wei25519_sign_verify(&session->signing_key, key_buf,
                                        ptr - key_buf, ptr)) {
...
static size_t raw_pickle_length(
    ...
    length += _olm_pickle_weisig25519_public_key_length(&session->signing_key);
...
size_t olm_pickle_inbound_group_session(
    ...
    pos = _olm_pickle_weisig25519_public_key(pos, &session->signing_key);
...
size_t olm_unpickle_inbound_group_session(
    ...
    pos = _olm_unpickle_weisig25519_public_key(pos, end, &session->signing_key);
    FAIL_ON_CORRUPTED_PICKLE(pos, session);
...
static size_t _decrypt(
     ...
     */
    message_length -= WEI25519_SIGNATURE_LENGTH;
    r = _olm_crypto_wei25519_sign_verify(
        &session->signing_key,
        message, message_length,
        message + message_length
    );
     ...
     * CRY.ME.VULN.19
     *
     * At this point, we call the signature verification function,
     * and we should check the result r, but we never do :)
     * (the following commented segment will be completely deleted)
        if (!r) {
            session->last_error = OLM_BAD_SIGNATURE;
            return (size_t)-1;
        }
    */
    ...
    size_t session_id_length = olm_inbound_group_session_id_length(session);
    session_id_ptr = (uint8_t *)malloc(session_id_length * sizeof(uint8_t));
    if(!session_id_ptr){
        session->last_error = OLM_MEMORY_ERROR;
        return (size_t)-1;
    }
    olm_inbound_group_session_id(session, session_id_ptr, session_id_length);
    ...
    r = megolm_cipher->ops->decrypt(
        ...
        decoded_results.version, decoded_results.message_index
    );

    _olm_unset(&megolm, sizeof(megolm));

    //session->last_error = OLM_AES_CBC_DECRYPT_BAD_INPUT_LENGTH_ERROR;
    //return (size_t)-1;
    /*if (r == (size_t)-1) {
        session->last_error = OLM_BAD_MESSAGE_MAC;
        return r;
    }*/
    if(r != EXIT_SUCCESS_CODE){
        if(r == CBC_DECRYPT_PADDING_FAILURE){
            session->last_error = OLM_AES_CBC_DECRYPT_PADDING_FAILURE_ERROR;
            return (size_t)-1;
        }
        else if(r == CBC_DECRYPT_BAD_INPUT_LENGTH){
            session->last_error = OLM_AES_CBC_DECRYPT_BAD_INPUT_LENGTH_ERROR;
            return (size_t)-1;
        }
        else if(r == MEMORY_ALLOCATION_ERROR){
            session->last_error = OLM_MEMORY_ERROR;
            return (size_t)-1;
        }
        else{
