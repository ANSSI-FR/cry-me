 ...
 * for that. The chain key and message key are both derived from SHA3
 * operations, so their length is determined by that. */
const std::size_t OLM_SHARED_KEY_LENGTH = SHA3_DIGEST_SIZE;
...
struct SessionId{
    std::uint8_t session_id[SHA3_DIGEST_SIZE];
};
...
struct OLM_EXPORT Ratchet {
    ...
    std::size_t encrypt_input_mac_length(
        std::size_t plaintext_length
    ) const;
