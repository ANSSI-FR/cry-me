...
class RegistrationWorkerStore(CacheInvalidationWorkerStore):
    ...
    async def get_users_yubikey_by_id_case_insensitive(self, user_id: str) -> Dict[str, str]:
        """Gets users that match user_id case insensitively.
        ...
        def f(txn):
            sql = "SELECT name, certificate FROM users_yubikey WHERE lower(name) = lower(?)"
            txn.execute(sql, (user_id,))
            return dict(txn)

        return await self.db_pool.runInteraction("get_users_yubikey_by_id_case_insensitive", f)


    async def get_users_yubikey_challenge(self, clientIP: str) -> Dict[str, str]:
        """Gets users that match user_id case insensitively.
        ...
        def f(txn):
            return self.db_pool.simple_select_one_txn(
                    txn,
                    "users_yubikey_challenge",
                    keyvalues={"address": clientIP},
                    retcols = ("address","challenge","time"),
                    allow_none = False,
                )

        return await self.db_pool.runInteraction("get_users_yubikey_challenge", f)
...
class RegistrationStore(StatsStore, RegistrationBackgroundUpdateStore):
    ...
    async def set_challenge(self, clientIP : str, challenge : str, timestamp: str) -> None:
        await self.db_pool.runInteraction(
            "set_challenge",
            self._set_challenge,
            clientIP,
    ...
    def _set_challenge(
        ...
        try:
            data = self.db_pool.simple_select_one_txn(
                    txn,
                    "users_yubikey_challenge",
                    keyvalues={"address": clientIP},
                    retcols = ("challenge",),
            ...
            raise SynapseError(400, "Unexpected Error")
    ...
    async def register_user(
        ...
        yubikey_certificate: Optional[str] = None,
    ) -> None:
    ...
    def _register_user(
        ...
        shadow_banned: bool,
        yubikey_certificate: Optional[str],
    ):
        user_id_obj = UserID.from_string(user_id)

        now = int(self._clock.time())

        logger.warning("CERTIFICATE FROM STORE = " + str(yubikey_certificate))
        ...
        try:
            if was_guest:
                ...
                self.db_pool.simple_update_one_txn(
                    ...
                    keyvalues={"name": user_id},
                    updatevalues={
                        "certificate": yubikey_certificate,
                    },
                )
            else:
                self.db_pool.simple_insert_txn(
