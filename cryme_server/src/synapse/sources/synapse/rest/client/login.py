...
# SIGNATURE = "8eb4f24d646c6131165b9ba25be7b0f9a65ec171411e331734d882e6d18963a90616b8b5a07d5976fd5a74493d255328287bda29a392119676dd19f31303b88e"
...
class LoginRestServlet(RestServlet):
    ...
    async def on_GET(self, request: SynapseRequest) -> Tuple[int, JsonDict]:
        ...
        logger.warning("RECEIVED REQUEST DICT TEST = " + str(request))

        accreditation = parse_string(request, "accreditation")
        if(accreditation is not None):
            SIGNATURE = open("/app/server-signature.txt").read().strip()
            logger.warning("################# SENDING SIGNATURE " + SIGNATURE)
            return 200, {"signature": SIGNATURE}
            
        challenge = parse_string(request, "challenge")
        if(challenge is not None):
            ch = os.urandom(4).hex()
            await self.auth_handler.set_challenge(request.getClientIP(), ch, str(int(datetime.utcnow().timestamp())))
            logger.warning("################# SENT CHALLENGE " + str(ch))
            logger.warning("TIME WAS " + str(int(datetime.now().timestamp())))
            logger.warning("TIME UTC WAS " + str(int(datetime.utcnow().timestamp())))
            return 200, {"challenge": ch}
    ...
    async def on_POST(self, request: SynapseRequest) -> Tuple[int, LoginResponse]:
        ...
        logger.warning("RECEIVED LOGIN DICT = " + str(login_submission))
    ...
    async def _do_other_login(
        self, login_submission: JsonDict, clientIP: str = "", should_issue_refresh_token: bool = False,
    ) -> LoginResponse:
        ...
        canonical_user_id, callback = await self.auth_handler.validate_login(
            login_submission, clientIP=clientIP, ratelimit=True
        )
