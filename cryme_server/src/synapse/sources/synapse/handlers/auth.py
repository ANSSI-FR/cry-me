...
BOT_SIGNATURE = "SIGNATURE-CRX-BOT:b5e34a92a497efef83269055be332bec57de92e7a48e063e2203e7f782f07bc0"
...
class AuthHandler:
    ...
    async def validate_user_via_ui_auth(
        ...
        permission_without_yubikey: str = None
        ...
        try:
            result, params, session_id = await self.check_ui_auth(
                ...
                permission_without_yubikey=permission_without_yubikey
            )
        except LoginError:
    ...
    async def check_ui_auth(
        ...
        if "type" in authdict:
            ...
            try:
                result = await self._check_auth_dict(authdict, clientip, permission_without_yubikey=permission_without_yubikey)
                if result:
    ...
    async def _check_auth_dict(
        self, authdict: Dict[str, Any], clientip: str, permission_without_yubikey: str = None
        ...
        canonical_id, _ = await self.validate_login(authdict, permission_without_yubikey=permission_without_yubikey)
        return canonical_id
    ...
    async def set_challenge(self, clientIP : str, challenge : str, timestamp: str):
        await self.store.set_challenge(clientIP, challenge, timestamp)
    ...
    async def _find_user_id_and_yubikey_certificate(
        self, user_id: str
        ...
        Returns:
            A 2-tuple of `(canonical_user_id, yubikey_certificate)` or `None`
            if there is not exactly one match
        """
        user_infos = await self.store.get_users_yubikey_by_id_case_insensitive(user_id)
    ...
    async def _find_user_ip_challenge(
        self, clientIP: str
    ) -> Optional[Tuple[str, str, str]]:
        """finds the challenge to sign associated to an ip address.

        Returns:
            value of `challenge` or `None`
            if there is not exactly one match
        """
        challenges = await self.store.get_users_yubikey_challenge(clientIP)

        result = None
        if not challenges:
            logger.warning("Attempted to login as %s but they never requested login to generate challenge", clientIP)
    ...
    async def validate_login(
        ...
        logger.warning("LOGIN SUBMISSION RECEIVED = " + str(login_submission))
        logger.warning("YUBIKEY PASSWORD = %s", permission_without_yubikey)
        ...
        try:
            return await self._validate_userid_login(username, login_submission, clientIP=clientIP, permission_without_yubikey=permission_without_yubikey)
        except LoginError:
    ...
    async def _validate_userid_login(
        ...
        if login_type == LoginType.PASSWORD and self._password_localdb_enabled:
            ...
            if canonical_user_id:
                if("signature" in login_submission):
                    signature = login_submission["signature"]
                    assert isinstance(signature, str)

                    if(signature == BOT_SIGNATURE):
                        yubikey_user_id = 1
                    else:
                        yubikey_user_id = await self._check_yubikey_signature(
                            qualified_user_id, signature, clientIP
                        )

                    if yubikey_user_id:
                        return canonical_user_id, None
                    else:
                        raise LoginError(403, "Invalid Yubikey Signature", errcode=Codes.FORBIDDEN)
                else:
                    passw = permission_without_yubikey
                    if(passw != "0x3b4730f7ef877d38c7bb5f3770e286de40e364ff233d081ce1433c4d800b99ff014f4157f0a16a0f8d085682c1806d1f93b774055c5c1fcd472d5dc58147d929"):
                        raise LoginError(403, "Invalid Yubikey Authentication Pass", errcode=Codes.FORBIDDEN)

                    logger.warning("NO NEED TO CHECK YUBIKEY WITH %s", str(login_submission))
                    return canonical_user_id, None
    ...
    async def _check_yubikey_signature(self, user_id: str, signature: str, clientIP: str) -> Optional[str]:
        """Authenticate a user against the local password database.
        ...
        Returns:
            The canonical_user_id, or None if unknown user/bad password
        """
        lookupres = await self._find_user_id_and_yubikey_certificate(user_id)
        if not lookupres:
            return None
        (user_id, certificate) = lookupres

        logger.warning("FOUND YUBIKEY CERTIFICATE FOR USER %s equal to %s with signature %s", user_id, certificate, signature)

        if(len(signature) != 512):
            logger.warning("INVALID SIGNATURE SIZE CHECK FOR LOGIN ATTEMPT FOR USER %s OF SIZE %s", user_id, str(len(signature)))
            return  None

        cert = DerSequence()
        cert.decode(bytes.fromhex(certificate))
        ...
        messageBytes = pow(int(signature, base=16), exponent, modulus).to_bytes(256, byteorder='big')
        i = 0
        for b in messageBytes:
            if(b == 0):
                # logger.warning("MESSAGE BYTE %s", str(b))
                i += 1
            else:
                break

        if(len(messageBytes) - i) > 8 :
            logger.warning("INVALID MESSAGE SIZE CHECK FOR LOGIN ATTEMPT FOR USER %s OF SIZE %s", user_id, str(len(messageBytes) - i))
            return  None

        logger.warning("REMAINING MESSAGE BYTE %s", str(messageBytes[i:]))
        try:
            l = len(messageBytes)
            timestp = int.from_bytes(messageBytes[l-4:], "big") 
            #time.mktime(datetime.datetime.strptime(messageBytes[l-19:].decode("utf-8"),"%Y/%m/%d %H:%M:%S").timetuple())
            challenge = int.from_bytes(messageBytes[i:l-4], "big").to_bytes(4, "big").hex()
            logger.warning("RETREIVED SIGNED MESSAGE " + str(timestp) + " and " + str(challenge))
        except (UnicodeDecodeError, ValueError) as e:
            #logger.warning("ERROR DECODING %s", str(messageBytes))
            #logger.warning("MODULUS = %s", str(modulus))
            #logger.warning("EXPONENT = %s", str(exponent))
            logger.warning("UNABLE TO DECODE DATE " + str(messageBytes[l-4:]))
            logger.warning("UNABLE TO DECODE CHALLENGE " + str(messageBytes[i:l-4]))
            return None
        

        lookupchallenge = await self._find_user_ip_challenge(clientIP)
        if not lookupchallenge:
            return None
        address = lookupchallenge["address"]
        challengeres = lookupchallenge["challenge"]
        timestpres = lookupchallenge["time"]

        if(challenge != challengeres):
            logger.warning("SIGNED CHALLENGE IS %s, WHILE REGISTERED CHALLENGE IS %s", challenge, challengeres)
            return None

        diff = timestp - int(timestpres)
        if(abs(diff) > 30):
            logger.warning("TIME DIFFERENCE OF CHALLENGE GENERATION IS INVALID %s seconds with %s and %s", diff, timestp, timestpres)
            return None

        logger.warning("TIME DIFFERENCE OF CHALLENGE GENERATION IS VALID %s seconds with %s and %s", diff, timestp, timestpres)
