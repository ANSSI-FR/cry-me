...
def get_verify_key_from_cross_signing_key(key_info):
    ...
    if len(keys) != 1:
        raise ValueError("Invalid key")
    logger.warning("PASSED KEY BASIC CHECKS JSON")
    # and return that one key
    for key_id, key_data in keys.items():
        logger.warning("ID = " + str(key_id))
        logger.warning("KEY = " + str(key_data))

        if key_id.startswith("weisig25519"):
            version = key_id[len("weisig25519") + 1 :]
            key: DefaultKey = DefaultKey(version, "weisig25519")
            key.version = version
            key.alg = "weisig25519"
            return key_id, key #decode_verify_key_bytes(key_id, decode_base64(key_data))
        else:
            raise ValueError("Invalid key")
